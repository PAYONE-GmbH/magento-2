...
class OrderTestTest extends BaseTestCase
{
    ...
    public function testFillSingleAddress()
    {
        ...
        $aResponse = [
            'add_paydata['.$sPrefix.'_firstname]' => 'Paul',
            'add_paydata['.$sPrefix.'_lastname]' => 'Tester',
            'add_paydata['.$sPrefix.'_street]' => 'Washington Blvd 13',
            'add_paydata['.$sPrefix.'_city]' => 'San Diego',
            'add_paydata['.$sPrefix.'_zip]' => '12345',
            'add_paydata['.$sPrefix.'_country]' => 'US',
        ];

        $region = $this->getMockBuilder(Region::class)->disableOriginalConstructor()->getMock();
        $region->method('getId')->willReturn('5');
        $this->customerHelper->method('getRegion')->willReturn($region);

        $result = $this->order->fillSingleAddressByResponse($address, $aResponse, true);
        $this->assertEquals($aResponse['add_paydata['.$sPrefix.'_firstname]'], $result->getFirstname());
    }
    ...
    public function testUpdateAddresses()
    {
        ...
        $aResponse = [
            ...
            'add_paydata['.$sPrefix.'_zip]' => '12345',
            'add_paydata['.$sPrefix.'_country]' => 'US',
            'add_paydata[email]' => 'tester@payone.com'
        ];
        ...
        $result = $this->order->updateAddresses($quote, $aResponse, false);
    ...
    public function testFillSingleAddressNoRegion()
    {
        ...
        $this->assertEquals($regionCode, $result->getRegionCode());
    }
}
