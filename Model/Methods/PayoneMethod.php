...
abstract class PayoneMethod extends BaseMethod
{
    ...
    /**
     * Returns if current payment methods has additional availability checks
     *
     * Extend this method if additional availability checks are added to the payment method
     *
     * @return bool
     */
    protected function hasExtendedAvailabilityChecks()
    {
        if (!empty($this->aAllowedCurrencies) ||
            $this->blIsB2BNotAllowed === true
        ) {
            return true;
        }
        return false;
    ...
    /**
     * Returns if given currency is allowed for current payment method
     *
     * @param \Magento\Quote\Api\Data\CartInterface|null $quote
     * @return bool
     */
    protected function isCurrencySupported(?\Magento\Quote\Api\Data\CartInterface $quote = null)
    {
        if (!empty($this->aAllowedCurrencies) && !empty($quote) && in_array($this->apiHelper->getCurrencyFromQuote($quote), $this->aAllowedCurrencies) === false) {
            return false;
        }
        return true;
    }
    ...
    {
        if (!empty($quote) && $quote->getBillingAddress() && !empty($quote->getBillingAddress()->getCompany())) {
            return true;
        }
        return false;
    }
    ...
    public function isAvailable(?\Magento\Quote\Api\Data\CartInterface $quote = null)
    {
        ...
        if ($blParentReturn === false || $this->hasExtendedAvailabilityChecks() === false) {
        ...
        if ($quote !== null) {
            if (!empty($this->aAllowedCurrencies) && $this->isCurrencySupported($quote) === false) {
                return false;
            }

            if ($this->blIsB2BNotAllowed === true && $this->isB2BOrder($quote) === true) {
                return false;
            }
        }
